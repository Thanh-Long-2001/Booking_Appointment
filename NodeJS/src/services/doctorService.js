...
let getAllDoctorsService = (page) => {
    
    const pageSize = 6; // Số lượng bản ghi trên mỗi trang
    const offset = (page - 1) * pageSize;

    return new Promise(async(resolve, reject) => {
        try {
            let totalDoctor = await db.User.findAll()
            ...
            const totalDoctors = totalDoctor.length;
...
let getDoctorByName = (searchInput) => {
            ...
            let doctors = await db.User.findAll({
                where: { 
                    ...
                    [Op.or]: [
                        { 
                            firstName: { 
                                [Op.like]: `%${searchInput.toLowerCase()}%`
                            } 
                        },
                        { 
                            lastName: { 
...
module.exports = {
    ...
    getAllDoctorsService2: getAllDoctorsService2,
