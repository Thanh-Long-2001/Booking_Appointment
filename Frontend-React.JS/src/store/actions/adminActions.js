...
import {
    ...
    getAllSpecialtyService, getAllClinicService2, getSpecialtyByNameService, getAllDoctorsService2,
...
export const fetchCreateNewUser = (data) => {
    return async (dispatch, getState) => {
        try {
            ...
            if (res && res.errCode === 0) {
                toast.warn("Vui lòng xác nhận email!")
...
//GET ALL DOCTORS
export const fetchGetAllDoctors = (page) => {
    return async (dispatch, getState) => {
        try {
            
            let res = await getAllDoctorsService(page)
            if (res && res.errCode === 0) {
                dispatch(fetchALLDoctorSuccess(res.data, res.pagination.totalPages))
            }
            else {
...
export const fetchALLDoctorSuccess = (data, totalPages) => ({
...
export const fetchGetAllDoctors2 = () => {
    return async (dispatch, getState) => {
        try {
            
            let res = await getAllDoctorsService2()
          
            if (res && res.errCode === 0) {
                dispatch(fetchALLDoctor2Success(res.data))
            }
            else {
                toast.error("Get all doctors error!")
                dispatch(fetchALLDoctor2Fail())
            }
        } catch (e) {
            toast.error("Get all doctors error!")
            dispatch(fetchALLDoctorFail())
...
export const fetchALLDoctor2Success = (data) => ({
    type: actionTypes.FETCH_ALL_DOCTORS2_SUCCESS,
    dataDr2: data,
    
})

export const fetchALLDoctor2Fail = () => ({
    type: actionTypes.FETCH_ALL_DOCTORS2_FAILED
})
//SAVE INFO DOCTORS
...
export const getRequiredDoctorInfo = () => {
    return async (dispatch, getState) => {
        try {
            ...
            let resClinic = await getAllClinicService2()
...
export const fetchGetAllHandbooks = (page) => {
    return async (dispatch, getState) => {
        try {
            
            let res = await getAllHandbookService(page)
            
            if (res && res.errCode === 0) {
                dispatch(fetchALLDoctorSuccess(res.data, res.pagination.totalPages))
            }
            else {
                toast.error("Get all handbooks error!")
                dispatch(fetchALLHandbookSuccess())
            }
        } catch (e) {
            toast.error("Get all handbooks error!")
...
export const fetchALLHandbookSuccess = (data, totalPages) => ({
